...
class TCNModel(nn.Module):
    ...
    def __init__(self, max_input_window_size, num_segments_exc, num_segments_inh, 
        ...
        for k in range(len(filter_sizes_per_layer)):
            ...
            padding = (filter_size - 1) * dilation_rate
            conv = nn.Conv1d(in_channels=current_channels, out_channels=num_filters,
                             kernel_size=filter_size, stride=stride, dilation=dilation_rate,
                             padding=padding, bias=True)
            
            # 初始化（以近似原TruncatedNormal stddev为参考）
            if isinstance(initializer, (int, float)):
                nn.init.trunc_normal_(conv.weight, std=initializer)
            else:
...
class SimulationDataGenerator:
    ...
    def _select_balanced_windows(self):
        ...
        if len(spike_rich_windows) < num_spike_rich and len(spike_rich_windows) > 0:
