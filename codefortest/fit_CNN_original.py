...
if synapse_type == 'NMDA':
    ...
    # train_data_dir = './Single_Neuron_InOut/data/L5PC_NMDA_train/'
    # valid_data_dir = './Single_Neuron_InOut/data/L5PC_NMDA_valid/'
    # test_data_dir  = './Single_Neuron_InOut/data/L5PC_NMDA_test/'
    # models_dir     = './Single_Neuron_InOut/models/NMDA/'

    train_data_dir = './Single_Neuron_InOut_SJC/data/L5PC_NMDA_train/'
    valid_data_dir = './Single_Neuron_InOut_SJC/data/L5PC_NMDA_valid/'
    test_data_dir  = './Single_Neuron_InOut_SJC/data/L5PC_NMDA_test/'
    models_dir     = './Single_Neuron_InOut_SJC/models/NMDA/depth_7_filters_128/'
...
loss_weights_per_epoch      = [[1.0, 0.0200]] * len(batch_size_per_epoch)
...
network_depth = 7 # 5
filter_sizes_per_layer        = [54,24,24,24,24,24,24]
num_filters_per_layer         = [128] * network_depth # 64
...
def parse_sim_experiment_file(sim_experiment_file, print_logs=False):
    ...
    # num_segments_exc  = experiment_dict['Params']['NUM_SYN_BASAL_EXC'] + experiment_dict['Params']['NUM_SYN_APIC_EXC']
    # num_segments_inh  = experiment_dict['Params']['NUM_SYN_BASAL_INH'] + experiment_dict['Params']['NUM_SYN_APIC_INH'] + experiment_dict['Params']['NUM_SYN_SOMA_INH']
    ...
    for k, sim_dict in enumerate(experiment_dict['Results']['listOfSingleSimulationDicts']):
        X_ex  = dict2bin(sim_dict['exInputSpikeTimes'], num_segments_exc, sim_duration_ms, 'exc')
        X_inh = dict2bin(sim_dict['inhInputSpikeTimes'], num_segments_inh, sim_duration_ms, 'inh')
...
class SimulationDataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, sim_experiment_files, num_files_per_epoch=10,
        ...
        self.batches_per_file = int(self.file_load * self.max_batches_per_file) # int(0.2*240)=48 (0.2*295)=59
    ...
    def __getitem__(self, batch_ind_within_epoch):
        ...
        selected_sim_inds = np.random.choice(range(self.num_simulations_per_file), size=self.batch_size, replace=True) # batch of 8 out of 128 simulations
        
        # randomly sample timepoints for current batch
        sampling_start_time = max(self.ignore_time_from_start, self.window_size_ms)
        selected_time_inds = np.random.choice(range(sampling_start_time, self.sim_duration_ms), size=self.batch_size, replace=False) # batch of 8 out of (6000-500) timepoints
    ...
    def load_new_file(self):
        ...
        # load the file
        X, y_spike, y_soma = parse_sim_experiment_file(self.curr_file_in_use)#, DVT_PCA_model=self.DVT_PCA_model)
        ...
        self.X, self.y_spike, self.y_soma = X, y_spike, y_soma #, y_DVT
...
# _, _, _, _ = parse_multiple_sim_experiment_files_with_DVT(train_files, DVT_PCA_model=DVT_PCA_model)
...
num_learning_schedules = len(batch_size_per_epoch) # 8
...
#         X_curr, y_spike_curr, y_soma_curr = parse_sim_experiment_file_with_DVT(sim_experiment_file, DVT_PCA_model=DVT_PCA_model)
